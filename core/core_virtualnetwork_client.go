...
// GetPrivateDnsRecordSet Gets the information for the specified `PrivateDnsRecordSet`.
func (client VirtualNetworkClient) GetPrivateDnsRecordSet(ctx context.Context, request GetPrivateDnsRecordSetRequest) (response GetPrivateDnsRecordSetResponse, err error) {
	var ociResponse common.OCIResponse
	policy := common.NoRetryPolicy()
	if request.RetryPolicy() != nil {
		policy = *request.RetryPolicy()
	}
	ociResponse, err = common.Retry(ctx, request, client.getPrivateDnsRecordSet, policy)
	if err != nil {
		if ociResponse != nil {
			response = GetPrivateDnsRecordSetResponse{RawResponse: ociResponse.HTTPResponse()}
		}
		return
	}
	if convertedResponse, ok := ociResponse.(GetPrivateDnsRecordSetResponse); ok {
		response = convertedResponse
	} else {
		err = fmt.Errorf("failed to convert OCIResponse into GetPrivateDnsRecordSetResponse")
	}
	return
}

// getPrivateDnsRecordSet implements the OCIOperation interface (enables retrying operations)
func (client VirtualNetworkClient) getPrivateDnsRecordSet(ctx context.Context, request common.OCIRequest) (common.OCIResponse, error) {
	httpRequest, err := request.HTTPRequest(http.MethodGet, "/privateDnsRecordSets/{privateDnsRecordSetId}")
	if err != nil {
		return nil, err
	}

	var response GetPrivateDnsRecordSetResponse
	var httpResponse *http.Response
	httpResponse, err = client.Call(ctx, &httpRequest)
	defer common.CloseBodyIfValid(httpResponse)
	response.RawResponse = httpResponse
...
// ListSubnetPrivateIPAddresses Lists the allocated private IP addresses mapped to the specified VNIC.
func (client VirtualNetworkClient) ListSubnetPrivateIPAddresses(ctx context.Context, request ListSubnetPrivateIPAddressesRequest) (response ListSubnetPrivateIPAddressesResponse, err error) {
	var ociResponse common.OCIResponse
	policy := common.NoRetryPolicy()
	if request.RetryPolicy() != nil {
		policy = *request.RetryPolicy()
	}
	ociResponse, err = common.Retry(ctx, request, client.listSubnetPrivateIPAddresses, policy)
	if err != nil {
		if ociResponse != nil {
			response = ListSubnetPrivateIPAddressesResponse{RawResponse: ociResponse.HTTPResponse()}
		}
		return
	}
	if convertedResponse, ok := ociResponse.(ListSubnetPrivateIPAddressesResponse); ok {
		response = convertedResponse
	} else {
		err = fmt.Errorf("failed to convert OCIResponse into ListSubnetPrivateIPAddressesResponse")
	}
	return
}

// listSubnetPrivateIPAddresses implements the OCIOperation interface (enables retrying operations)
func (client VirtualNetworkClient) listSubnetPrivateIPAddresses(ctx context.Context, request common.OCIRequest) (common.OCIResponse, error) {
	httpRequest, err := request.HTTPRequest(http.MethodGet, "/subnets/{subnetId}/privateIPAddresses")
	if err != nil {
		return nil, err
	}

	var response ListSubnetPrivateIPAddressesResponse
	var httpResponse *http.Response
	httpResponse, err = client.Call(ctx, &httpRequest)
	defer common.CloseBodyIfValid(httpResponse)
	response.RawResponse = httpResponse
